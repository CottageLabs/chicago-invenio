...
from chicago_invenio.scripts.community_assignment_algorithm import (
    CommunityAssignmentAlgorithm,
)
...
def parse_personal_name_from_text(name_text: str) -> Dict[str, Any]:
    ...
    person_data = {"type": "personal", "name": name_text}
...
def parse_corporate_name(name_field) -> Dict[str, Any]:
    ...
    return {
        "person_or_org": {"type": "organizational", "name": subfield_a.text.strip()}
    }
...
def scrape_doi_publication_date(doi: str) -> Optional[str]:
    ...
    try:
        ...
        pub_date_match = re.search(
            r"<strong>Published\s+(\d{1,2})\s+(\w+),?\s+(\d{4})</strong>",
            html,
            re.IGNORECASE,
        )
        if pub_date_match:
    ...
    if content_type_field is not None:
        ...
        if content_type_a is not None:
            ...
            for key, value in sorted(
                RESOURCE_TYPE_MAPPINGS.items(), key=lambda x: len(x[0]), reverse=True
            ):
...
def extract_ror_id_from_identifier(identifier: str) -> Optional[str]:
    ...
    if "ror.org/" in identifier.lower():
        logger.debug(
            f"extract_ror_id_from_identifier: ROR pattern detected '{identifier}'"
        )
        ...
        if ror_match:
            ...
            if extracted_id in INVALID_ROR_IDS:
                logger.debug(
                    f"extract_ror_id_from_identifier: ignoring invalid ROR ID '{extracted_id}'"
                )
                return None
            # Validate the extracted ID
            if idutils.is_ror(extracted_id):
                logger.debug(
                    f"extract_ror_id_from_identifier: valid ROR ID '{extracted_id}'"
                )
                return extracted_id
            else:
                logger.debug(
                    f"extract_ror_id_from_identifier: invalid ROR ID '{extracted_id}'"
                )
    ...
    # Check if it's already a plain ROR ID (not a URL)
        ...
        if identifier in INVALID_ROR_IDS:
            logger.debug(
                f"extract_ror_id_from_identifier: ignoring invalid ROR ID '{identifier}'"
            )
            return None
        return identifier

    logger.debug(
        f"extract_ror_id_from_identifier: no valid ROR ID found in '{identifier}'"
    )
    return None
...
def parse_marc_record(record_elem, record_identifier) -> Dict[str, Any]:
    ...
    for personal_field in added_personal:
        ...
        if subfield_a is not None:
            ...
            if any(word in name_text.lower() for word in ORG_WORDS):
                ...
                creator_data = {
                    "person_or_org": {"type": "organizational", "name": name_text}
                }
                # Add affiliation if present
    ...
    for tag in ["701", "702", "703", "712", "713"]:
        variant_fields = record_elem.findall(
            f'.//marc:datafield[@tag="{tag}"]', MARC_NS
        )
    ...
    contributor_720_fields = record_elem.findall(
        './/marc:datafield[@tag="720"]', MARC_NS
    )
        ...
        if contrib_name is not None:
            ...
            if any(word in name_text.lower() for word in ORG_WORDS):
                ...
                if parsed_name:
                    ...
                    contributor_data = {
                        "person_or_org": {"type": "personal", "name": name_text},
                        "role": {"id": role},
                    }
            ...
            if contrib_affiliation is not None:
                contributor_data["affiliations"] = [
                    {"name": contrib_affiliation.text.strip()}
                ]
    ...
    if contributors:
        ...
        for contributor in contributors:
            ...
            if contributor_key not in seen_contributors:
                ...
                existing_role = (
                    seen_contributors[contributor_key]
                    .get("role", {})
                    .get("id", "other")
                )
    ...
    if not creators:
        ...
        if title_field is not None:
            ...
            if title_c is not None:
                ...
                creator_name = (
                    creator_name.replace("by ", "").replace("By ", "").strip()
                )
                if creator_name:
                    creators = [
                        {"person_or_org": {"type": "personal", "name": creator_name}}
                    ]

    # Final fallback - create unknown creator
    if not creators:
        creators = [{"person_or_org": {"type": "organizational", "name": "Unknown"}}]
    ...
    if title_field is not None:
        ...
        title = (
            strip_html_tags(title_a.text.strip()) if title_a is not None else "Untitled"
        )
    ...
    for variant_field in variant_title_fields:
        ...
        if (
            variant_title_a is not None
            and variant_title_a.text
            and variant_title_a.text.strip()
        ):
            title_entry = {
    ...
    identifier_024_fields = record_elem.findall(
        './/marc:datafield[@tag="024"]', MARC_NS
    )
    for id_field in identifier_024_fields:
        ...
        if (
            id_field.get("ind1") == "7"
            and subfield_2 is not None
            and subfield_2.text.strip().lower() == "doi"
            and subfield_a is not None
        ):
            doi_value = subfield_a.text.strip()
            # Validate DOI using idutils
            if idutils.is_doi(doi_value):
                identifiers.append({"identifier": doi_value, "scheme": "doi"})
            else:
        ...
        # Handle patent numbers (MARC 024$a)
            ...
            if id_value and not any(
                existing["identifier"] == id_value for existing in identifiers
            ):
                ...
                if idutils.is_doi(id_value):
                    identifiers.append({"identifier": id_value, "scheme": "doi"})
                # Assign as patent number
                else:
                    identifiers.append(
                        {"identifier": id_value, "scheme": "patent_number"}
                    )
        ...
        if subfield_d is not None:
            ...
            if id_value and not any(
                ...
                identifiers.append(
                    {"identifier": id_value, "scheme": "patent_application_number"}
                )
    ...
    for url_field in url_fields:
        ...
        if url_u is not None:
            ...
            if url_text.startswith("http") and "knowledge.uchicago.edu" not in url_text:
                identifiers.append({"identifier": url_text, "scheme": "url"})

    # OAI identifiers (MARC 909CO)
    oai_fields = record_elem.findall(
        './/marc:datafield[@tag="909"][@ind1="C"][@ind2="O"]', MARC_NS
    )
    for oai_field in oai_fields:
        ...
        if oai_o is not None:
            ...
            if oai_text.startswith("oai:"):
                identifiers.append({"identifier": oai_text, "scheme": "other"})
        ...
        if date_d is not None:
            ...
            if date_value:
                ...
                if date_type_o is not None:
                    ...
                    content_type_field = record_elem.find(
                        './/marc:datafield[@tag="336"]', MARC_NS
                    )
                    if content_type_field is not None:
                        content_type_a = content_type_field.find(
                            './/marc:subfield[@code="a"]', MARC_NS
                        )
                        if (
                            content_type_a is not None
                            and content_type_a.text.strip().lower() == "interview"
                        ):
                            date_entry["type"] = {
                                "id": "collected",
                                "title": {"en": "Collected"},
    ...
    if "publication_date" not in metadata:
        ...
        if "publication_date" not in metadata:
            copyright_field = record_elem.find(
                './/marc:datafield[@tag="264"][@ind2="4"]', MARC_NS
            )
            if copyright_field is not None:
                copyright_c = copyright_field.find(
                    './/marc:subfield[@code="c"]', MARC_NS
                )
        ...
        if "publication_date" not in metadata:
            ...
            if creation_field is not None:
                creation_k = creation_field.find(
                    './/marc:subfield[@code="k"]', MARC_NS
                )  # Beginning date
    ...
    for desc_field in desc_fields_520:
        ...
        if desc_a is not None:
            ...
            if desc_text.lower() != "no abstract":
                ...
                if not main_description_set and (
                    desc_type_7 is None
                    or desc_type_7.text.strip().lower() in ["abstract", ""]
                ):
            ...
            additional_descriptions.append(
                {
                    ...
                    "type": {
                        ...
                        "title": {"en": "Data Availability Statement"},
                    },
                }
            )
    ...
    if rights_542 is not None:
        ...
        if license_a is not None:
            ...
            if license_id.lower() in LICENSE_MAPPINGS:
                ...
                rights.append({"id": LICENSE_MAPPINGS[license_id.lower()]})
            else:
        ...
        if license_f is not None and license_a is None:
            ...
            if rights_text.lower() in LICENSE_MAPPINGS:
                # Use only id for known licenses
                rights.append({"id": LICENSE_MAPPINGS[rights_text.lower()]})
            else:
                if not rights_text.lower().startswith(
                    "university of chicago dissertations are covered by copyright."
                ) and not rights_text.lower().startswith(
                    "university of chicago theses are covered by copyright."
                ):
    ...
    related_856_fields = record_elem.findall(
        './/marc:datafield[@tag="856"][@ind1="4"][@ind2="0"]', MARC_NS
    )
    ...
    archive_857_fields = record_elem.findall(
        './/marc:datafield[@tag="857"][@ind1="4"][@ind2="0"]', MARC_NS
    )
    ...
    if thesis_field is not None:
        ...
        if thesis_type is not None:
            custom_fields["thesis:thesis"] = {"type": thesis_type.text.strip()}
        ...
        if ror_id is not None:
            ...
            if ror_id:
                ...
                if funder_name is not None:
                    ...
                    logger.debug(
                        f"Setting funder name to: '{funder_name.text.strip()}'"
                    )
    ...
    for geo_field in geo_522_fields:
        ...
        if place_a is not None:
            locations.append({"features": [{"place": place_a.text.strip()}]})
    ...
    for geo_field in geo_975_fields:
        place_a = geo_field.find('.//marc:subfield[@code="a"]', MARC_NS)
        coord_b = geo_field.find(
            './/marc:subfield[@code="b"]', MARC_NS
        )  # latitude for point (-118.1)
        coord_c = geo_field.find(
            './/marc:subfield[@code="c"]', MARC_NS
        )  # longitude for point (34.1)
        coord_d = geo_field.find(
            './/marc:subfield[@code="d"]', MARC_NS
        )  # coord1 for polygon (-104.05)
        coord_e = geo_field.find(
            './/marc:subfield[@code="e"]', MARC_NS
        )  # coord2 for polygon (-111.05)
        coord_f = geo_field.find(
            './/marc:subfield[@code="f"]', MARC_NS
        )  # coord3 for polygon (45.01)
        coord_g = geo_field.find(
            './/marc:subfield[@code="g"]', MARC_NS
        )  # coord4 for polygon (40.99)
        ...
        if coord_b is not None and coord_c is not None:
            try:
                ...
                logger.warning(
                    f"Invalid coordinates in MARC 975: lat={coord_c.text}, lng={coord_b.text}"
                )
    ...
    embargo_fields = record_elem.findall(
        './/marc:datafield[@tag="856"][@ind1="4"]', MARC_NS
    )
...
def add_files_to_draft(draft: RecordItem, identity, filepath: str):
    ...
    try:
        ...
        record_file_path = os.path.join(
            filepath,
            str(draft.data.get("custom_fields", {}).get("chicago:tind_id", "")),
        )

        # Check if test file exists
        if not os.path.exists(record_file_path):
            logger.warning(
                f"Directory not found at {record_file_path}, skipping file upload"
            )
            return
    ...
    for record_elem in records_batch:
        ...
        try:
            ...
            draft = current_rdm_records_service.create(
                data=invenio_data, identity=identity
            )
            ...
            if community_algorithm:
                ...
                logger.info(
                    f"Record {published.id} field values: divisions={divisions}, departments={departments}, centers={centers}, resource_type={resource_type}"
                )
                logger.info(
                    f"Algorithm returned {len(assignment_result.get('communities', []))} communities: {assignment_result.get('communities', [])}"
                )
                ...
                if assignment_result.get("communities"):
                    for community_name in assignment_result["communities"]:
                        ...
                        logger.info(
                            f"Looking up community '{community_name}' in community_map"
                        )
                        if community_name in community_map:
                            community_id = community_map[community_name]
                            logger.info(
                                f"Found community '{community_name}' with ID {community_id}"
                            )
                            ...
                            try:
                                request_id = current_record_communities_service.add(
                                    ...
                                    dict(
                                        communities=[
                                            dict(id=community_id, require_review=False)
                                        ]
                                    ),
                                )[0][0]["request_id"]
                                ...
                                logger.info(
                                    f"Added record {published.id} to community '{community_name}' (ID: {community_id})"
                                )

                            except Exception as community_error:
                                logger.warning(
                                    f"Failed to add record {published.id} to community '{community_name}': {community_error}"
                                )
                        else:
                            logger.warning(
                                f"Community '{community_name}' not found in community_map"
                            )

                    # Log assignment details
                    logger.info(
                        f"Record {published.id} assigned to {len(assignment_result['communities'])} communities using method '{assignment_result['method']}'"
                    )
                else:
                    logger.info(
                        f"No communities assigned for record {published.id}: {assignment_result.get('method', 'unknown method')}"
                    )
            else:
                logger.debug(
                    f"Community algorithm not available for record {published.id} - skipping community assignment"
                )
            ...
            logger.error(
                f"Error processing record {record_identifier} at stage {error_record['error_stage']}: {e}"
            )
            import traceback
...
@click.option(
    "--batch-size", default=500, help="Number of records to process in each batch"
)
@click.option(
    "--max-records",
    default=3000,
...
def import_data(
    email: str, data: str, filepath: str, batch_size: int, max_records: Optional[int]
):
    ...
    with app.app_context():
        ...
        try:
            ...
            while True:
                ...
                # Process the batch
                batch_results = process_records_batch(
                    batch, identity, community_map, filepath, community_algorithm
                )
